...
import com.swind.cloud.hotel.v2.shiro.LoginType;
import com.swind.cloud.hotel.v2.shiro.UserToken;
...
//        UserToken token = new UserToken(LoginType.USER_PASSWORD, params.get("username").toString(), params.get("password").toString());
        UsernamePasswordToken usernamePasswordToken = new UsernamePasswordToken(
                params.get("username").toString(),
                params.get("password").toString());
        return shiroLogin(usernamePasswordToken);
    }
    ...
    public Result shiroLogin(UsernamePasswordToken token){
        try {
            //登录不在该处处理，交由shiro处理
    ...
    @ResponseBody
    public Result getUserAction() {
        JSON json = new JSONObject();
        ((JSONObject) json).put("111", 1111);
        return new Result(ResultStatusCode.OK, json);
    }
}
